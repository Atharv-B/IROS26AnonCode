...
class ExampleFullArmMovement:
    ...
    def controller_callback(self, data):
        ...
        if controller_buttons[3] == 1:
            print("RESET ROBOT")
            self.example_send_joint_angles([266.93, 46.585, 172.239, 240.611, 257.755, 74.649, 103.245])
            # res = self.send_robot_position(10006)
            # print(res)
    ...
    def df_send_joint_angles(self, df_positions):
        # print(df_positions)
        self.last_action_notif_type = None
        ...
        for index, position in df_positions.iterrows():
            ...
            angularWaypoint.angles.append(float(position['Position 0']))
            angularWaypoint.angles.append(float(position['Position 1']))
            angularWaypoint.angles.append(float(position['Position 2']))
            angularWaypoint.angles.append(float(position['Position 3']))
            angularWaypoint.angles.append(float(position['Position 4']))
            angularWaypoint.angles.append(float(position['Position 5']))
            angularWaypoint.angles.append(float(position['Position 6']))
