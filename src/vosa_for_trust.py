...
class VOSATeleoperation(SAGTeleoperation):
    def __init__(self):
        ...
        # Override task param
        self.task = "shelving"  # Default to "shelving" task for VOSA
            ...
            if self.input_magnitude(self.uh) > 1e-6:
                ...
                if self.task == "shelving":
                    ...
                    cmd[2] *= (z_mult_place if self.gripper_closed else z_mult_pick)
                     ...
                     # If placing at high Z position and still far away, prioritize Z movement first
                    if self.gripper_closed and goal_idx < len(self.place_set_mod):
                        target_z = self.place_set_mod[goal_idx][2]
                        current_z = self.ee_position[2]
                        z_error = abs(target_z - current_z)
                        z_threshold = rospy.get_param("~high_z_threshold", 0.40)
                        z_error_threshold = rospy.get_param("~z_error_threshold", 0.10)
                        
                        # Only move in Z if target is high AND we're still far from target Z
                        if target_z > z_threshold and z_error > z_error_threshold:
                            rospy.loginfo_throttle(1.0, f"[VOSA] High shelf (target_z={target_z:.3f}, error={z_error:.3f}): Z-only movement")
                            # Zero out X and Y commands for high placements
                            cmd[0] = 0.0
                            cmd[1] = 0.0
